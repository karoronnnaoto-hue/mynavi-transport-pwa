...
初回に母集団を作る場合は、`Discover new internships` を手動実行し、`full_scan` を `true` にします。これによりカタログ済みの詳細取得数制限を外し、詳細取得後に「交通費あり」の募集だけを画面表示・金額分析の対象にします。通常の定期巡回は負荷を抑えるため、少しずつ新規募集を確認します。
...
- 一覧ページの「次の30社」は通常の `href` ではなく、`searchCorpListByIsForm` をJavaScriptでPOSTする導線です。requestsだけで単純再現するとマイナビ側で「不正な操作」となったため、ページ送りの完全自動化はブラウザ相当の状態再現が必要です。
