...
初回に母集団を作る場合は、`Discover new internships` を手動実行し、`full_scan` を `true` にします。これにより一覧を最後まで巡回し、詳細取得後に「交通費あり」の募集だけを画面表示・金額分析の対象にします。通常の定期巡回は負荷を抑えるため、少しずつ新規募集を確認します。
